                    ...
                    @component('components.table',['class'=> 'lf-data-table'])
                        ...
                        @foreach($dataTable['items'] as $withdrawal)
                            <tr>
                                ...
                                <td class="text-right">{{ ($withdrawal->amount - $withdrawal->system_fee) }} {{ $withdrawal->symbol }}</td>
                                <td>{{ $withdrawal->payment_method ? payment_methods($withdrawal->payment_method) : '-' }}</td>
                                <td class="text-center">
                                    <span
                                        class="font-size-12 py-1 px-2 badge badge-{{ get_color_class($withdrawal->status,'payment_status') }}">
                                    {{ payment_status($withdrawal->status) }}
                                </span>
                                </td>
                                {{--                                <td>{{ $withdrawal->payment_method === API_BANK ? $withdrawal->bankAccount->bank_name : $withdrawal->id }}</td>--}}
                                <td>{{ $withdrawal->id }}</td>
