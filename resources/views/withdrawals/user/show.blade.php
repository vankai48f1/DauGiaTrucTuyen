                ...
                <div class="card lf-toggle-bg-card lf-toggle-border-color p-4 mb-5">
                    <div class="card-header lf-toggle-border-color px-0 pb-3 pt-0">
                        <div class="d-block">
                            ...
                            @if(has_permission('wallets.withdrawals.destroy') &&
                        in_array($withdrawal->status, [PAYMENT_STATUS_PROCESSING, PAYMENT_STATUS_PENDING, PAYMENT_STATUS_REVIEWING]))
                                <a href="{{ route('wallets.withdrawals.destroy', ['wallet' => $withdrawal->currency_symbol, 'withdrawal' => $withdrawal->id]) }}"
                        ...
                        <div class="table-responsive">
                            <table class="table table-borderless">
                                <tr>
                                    ...
                                    <td>{{ $withdrawal->currency->name }} ({{ $withdrawal->currency_symbol }})</td>
                                    <th class="pl-0">{{__("Status")}}</th>
                                    <td>
                                        <span
                                            class="badge badge-{{ config('commonconfig.payment_status.'.$withdrawal->status.'.color_class') }}">{{ payment_status($withdrawal->status) }}</span>
                                    </td>
                                </tr>
                                <tr>
                                ...
                                <table class="table table-borderless">
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            {{ verification_status($withdrawal->bankAccount->is_verified) }}
                                            </span>
                                        </td>
                                    </tr>
                                    <tr>
