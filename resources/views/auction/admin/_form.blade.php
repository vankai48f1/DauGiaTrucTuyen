<div class="card">
    ...
    <div class="card-body p-5">
        ...
        <div class="form-group form-row">
            ...
            <div class="col-lg-10">
                <!-- Start: currency type -->
                {{ Form::select('currency_symbol', $currencies, $auction->currency_symbol, ['class' => 'custom-select color-666', 'id' => 'currency_symbol', 'placeholder' =>  __('Select Currency Type'), 'disabled' => $auction->status != AUCTION_STATUS_DRAFT]) }}
                <span class="invalid-feedback" data-name="currency_symbol">{{ $errors->first('currency_symbol') }}</span>
                <!-- End: currency type -->


                <!-- Start: auction type -->
                {{ Form::select('auction_type', auction_type(), $auction->auction_type, ['class' => 'custom-select color-666 mt-3', 'id' => 'auction_type', 'placeholder' =>  __('Select Auction Type'), 'disabled' => $auction->status != AUCTION_STATUS_DRAFT]) }}
        ...
        <div class="form-group form-row mt-4">
            ...
            <div class="col-lg-10">
                ...
                <div class="form-row">
                    <div class="col-12">
                        <!-- Start: title -->
                        {{ Form::text('title', null, ['class'=> 'form-control', 'id' => 'title', 'placeholder' => __('Contest Title')]) }}
                ...
                <div class="form-row mt-3">
                    ...
                    <div class="col-6">
                        <p class="form-control">{{$auction->bid_initial_price}}</p>
                    </div>
                    <!-- End: bid initial price -->

                    <!-- Start: bid increment difference -->
                    <div v-if="showBidIncrement === '{{AUCTION_TYPE_HIGHEST_BIDDER}}'" class="col-6">
                        {{ Form::text('bid_increment_dif', null, ['class'=> 'form-control', 'id' => 'bid_increment_dif', 'placeholder' => __('Bid Increment Difference')]) }}
                        <span class="invalid-feedback" data-name="bid_increment_dif">{{ $errors->first('bid_increment_dif') }}</span>
                    </div>
                ...
                <div class="form-row mt-3">
                    <div class="col-6">
                        ...
                        <!-- Start: starting date -->
                        <p class="form-control">{{$auction->starting_date}}</p>
                        <!-- End: starting date -->

                    </div>
                    <div class="col-6">
                        <!-- Start: ending date -->
                        {{ Form::text('ending_date', null, ['class'=> 'form-control datepicker', 'id' => 'ending_date', 'placeholder' => __('Ending Date')]) }}
                        <span class="invalid-feedback" data-name="ending_date">{{ $errors->first('ending_date') }}</span>
                        <!-- End: ending date -->
                    </div>
                </div>
        ...
        <div class="form-group form-row mt-4">
            ...
            <div class="col-lg-10">
                ...
                <!-- Start: description -->
                {{ Form::textarea('product_description', null, ['class'=> 'form-control', 'id' => 'product_description', 'placeholder' => __('Description'), 'rows' => '3']) }}
        ...
        <div class="form-group form-row mt-4">
            {{Form::label('main_content', __('Terms Description :'), ['class' => 'col-lg-2 col-form-label text-right pr-3 color-999'])}}
            <div class="col-lg-10">

                <!-- Start: terms description -->
                {{ Form::textarea('terms_description', null, ['class'=> 'form-control', 'id' => 'terms_description', 'placeholder' => __('Terms Description'), 'rows' => '3']) }}
        ...
        <div class="form-group form-row mt-4">
            {{Form::label('main_content', __('Shippable :'), ['class' => 'col-lg-2 col-form-label text-right pr-3 color-999'])}}

            <div class="col-md-10">
                <div class="lf-switch">

                    {{ Form::radio('is_shippable', ACTIVE_STATUS_ACTIVE, (string)old('is_shippable', isset($paymentMethod) ? $paymentMethod->is_shippable : ACTIVE_STATUS_ACTIVE) === (string)ACTIVE_STATUS_ACTIVE ? true : false, ['id' => 'is_shippable' . '-active', 'class' => 'lf-switch-input',]) }}
                    <label for="{{ 'is_shippable'}}-active" class="lf-switch-label">{{ __('Yes') }}</label>

                    {{ Form::radio('is_shippable', ACTIVE_STATUS_INACTIVE, (string)old('is_shippable', isset($paymentMethod) ? $paymentMethod->is_shippable : false) === (string)ACTIVE_STATUS_INACTIVE ? true : false, ['id' => 'is_shippable' . '-inactive', 'class' => 'lf-switch-input']) }}
        ...
        <!-- Start: basic information -->
        <div class="form-group form-row mt-4">
            {{Form::label('content', __('Shipping Type :'), ['class' => 'col-lg-2 col-form-label text-right pr-3 color-999'])}}

            <div class="col-md-10">
                <div class="lf-switch">
                    {{ Form::radio('shipping_type', ACTIVE_STATUS_ACTIVE, (string)old('shipping_type', isset($paymentMethod) ? $paymentMethod->shipping_type : ACTIVE_STATUS_ACTIVE) === (string)ACTIVE_STATUS_ACTIVE ? true : false, ['id' => 'shipping_type' . '-active', 'class' => 'lf-switch-input',]) }}
                    <label for="{{ 'shipping_type' }}-active" class="lf-switch-label">{{ __('Free') }}</label>

                    {{ Form::radio('shipping_type', ACTIVE_STATUS_INACTIVE, (string)old('shipping_type', isset($paymentMethod) ? $paymentMethod->shipping_type : false) === (string)ACTIVE_STATUS_INACTIVE ? true : false, ['id' => 'shipping_type' . '-inactive', 'class' => 'lf-switch-input']) }}
        ...
        <!-- Start: basic information -->
        <div class="form-group form-row mt-4">
            {{Form::label('content', __('Multiple Bid :'), ['class' => 'col-lg-2 col-form-label text-right pr-3 color-999'])}}

            <div class="col-md-10">
                <div class="lf-switch">
                    {{ Form::radio('is_multiple_bid_allowed', ACTIVE_STATUS_ACTIVE, (string)old('is_multiple_bid_allowed', isset($paymentMethod) ? $paymentMethod->is_multiple_bid_allowed : ACTIVE_STATUS_ACTIVE) === (string)ACTIVE_STATUS_ACTIVE ? true : false, ['id' => 'is_multiple_bid_allowed' . '-active', 'class' => 'lf-switch-input',]) }}
                    <label for="{{ 'is_multiple_bid_allowed' }}-active" class="lf-switch-label">{{ __('Allowed') }}</label>

                    {{ Form::radio('is_multiple_bid_allowed', ACTIVE_STATUS_INACTIVE, (string)old('is_multiple_bid_allowed', isset($paymentMethod) ? $paymentMethod->is_multiple_bid_allowed : false) === (string)ACTIVE_STATUS_INACTIVE ? true : false, ['id' => 'is_multiple_bid_allowed' . '-inactive', 'class' => 'lf-switch-input']) }}
                    <label for="{{ 'is_multiple_bid_allowed' }}-inactive" class="lf-switch-label">{{ __('Not Allowed') }}</label>
                </div>
            </div>
        </div>
        <!-- End: basic information -->
        ...
        <!-- Start: product image -->
        <div class="form-group form-row mt-4">
            <label class="col-lg-2 col-form-label text-right pr-3 color-999" for="{{ 'content' }}">{{('Multiple Image :')}}</label>
            <div class="col-lg-10">
                <div id="preview-multi-img">
                    ...
                    <button id="btnAdd" type="button" class="btn btn-info mt-3 btn-sm font-size-12" data-toggle="tooltip">{{__('Add Next Image')}}</button>
                </div>
