                            ...
                            <tr>
                                ...
                                <td>
                                    @if($ticket->previous_id)
                                        <div class="border-bottom d-flex mb-3 pb-3 align-items-center">
                                            <a target="_blank"
                                               href="{{ route('admin.tickets.show', $ticket->previous_id) }}">{{$ticket->previous_id}}</a>
                                        </div>
                                    @endif
                                </td>
                                <td class="text-center">
                                    @if($ticket->assigned_to)
                                        <a target="_blank"
                                           href="{{ route('admin.users.show', $ticket->assigned_to) }}">{{ $ticket->assigned_user_full_name }}</a>
                                    @else
                                        -
                                    @endif
                                </td>
                                ...
                                <td class="text-center"><small
                                        class="rounded-pill py-1 px-2 badge badge-{{ config('commonconfig.ticket_status.'.$ticket->status.'.color_class') }}">{{ ticket_status($ticket->status) }}</small>
                                </td>
