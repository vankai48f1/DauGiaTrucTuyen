    ...
    <div class="container py-5">
        <div class="row">
            ...
            <div class="col-lg-4 order-lg-1 mb-lg-0 mb-4">
                @component('components.card',['headerClass' => 'bg-light mb-2'])
                    ...
                    <div class="border-bottom d-flex mb-3 pb-3 align-items-center">
                        ...
                        <div class="text-muted">
                            @if(optional($ticket->assignedUser)->profile)
                                <a href="{{ route('admin.users.show',$ticket->assigned_to) }}"
                                   target="_blank">{{ optional(optional($ticket->assignedUser)->profile)->full_name}}</a>
                            @else
                                -
                            @endif
                        </div>
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <small
                            class="rounded-pill px-2 py-1 badge badge-{{ config('commonconfig.ticket_status.'. $ticket->status.'.color_class') }}">{{ ticket_status($ticket->status) }}</small>
                    </div>
                @endcomponent

                @if(in_array($ticket->status,[STATUS_OPEN,STATUS_IN_PROGRESS]))
                    @component('components.card',['class'=> 'mt-4','headerClass' => 'bg-light mb-2'])
                        @slot('header')
                        ...
                        @if($ticket->status == STATUS_OPEN)
                            @if(is_null($ticket->assigned_to))
                                <form
                                    ...
                                    <div class="form-group">
                                        {{ Form::submit(__('Assign'), ['class' => 'btn btn-block btn-primary form-submission-button']) }}
                                    </div>
            ...
            <div class="col-lg-8 order-lg-0">
                @component('components.card',['type' => 'info','headerClass' => 'bg-light','bodyClass'=> 'pt-4'])
                    @slot('header')
                ...
                @includeWhen(($ticket->status == STATUS_IN_PROGRESS),'core.tickets._comment_form')
            </div>
        </div>
    </div>
@endsection
...
@section('style')
    ...
    <link rel="stylesheet" href="{{ asset('public/plugins/select2/css/select2-bootstrap4.min.css') }}">
