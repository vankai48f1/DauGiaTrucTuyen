<div class="light-header py-3 px-3">
    <form action="{{$route}}" class="lf-filter-form" method="get">
        <div class="d-md-flex">
            @if(isset($orderFields) && !empty($orderFields))
                <div class="lf-flex-1 ml-md-1">
                    <div class="form-group d-flex">
                        <div class="lf-select lf-flex-1">
                            <select class="form-control lf-filter-sort-by" name="{{$pageName}}-sort">
                        ...
                        <div class="lf-select ml-1 lf-flex-1">
                            <select class="form-control lf-filter-order-by" name="{{$pageName}}-ord">
                                <option value="d"{{return_get($pageName.'-ord','d')}}>{{ __('Desc') }}</option>
                                <option value="a"{{return_get($pageName.'-ord','a')}}>{{ __('Asc') }}</option>
                            </select>
                        </div>
                        <button type="submit" class="auctioneer-primary-color btn btn-danger ml-1"><i class="fa fa-arrow-right"></i></button>
                    </div>
                </div>
            @endif
                ...
                <div class="lf-flex-2 ml-md-1">
                    <div class="d-flex lf-filter-search-group{{ $displayFilterButton ? ' lf-filter-search-mr' : '' }}">
                        <div class="lf-select lf-flex-3">
                            <select class="form-control" name="{{$pageName}}-ssf">
                        ...
                        <div class="lf-flex-5 ml-1">
                            <input type="text" class="form-control lf-filter-search mr-0" name="{{$pageName}}-srch"
                                   placeholder="search"
                                   value="{{return_get($pageName.'-srch')}}">
                        </div>
                        <button type="submit" class="auctioneer-primary-color btn btn-danger ml-1"><i class="fa fa-search"></i></button>
                        @if($displayFilterButton)
                            <a href="javascript:" class="lf-filter-toggler ml-1 btn btn-warning">
                                ...
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right" role="menu">
                                        @foreach(datatable_downloadable_type() as $item => $value)
                                            <a class="dropdown-item download" data-type="{{ $item }}"
                                               href="{{ url()->current() }}"><i
                        ...
                        @if(is_array($value))
                            @foreach($value as $optionKey=>$optionValue)
                                @if(!empty($optionValue) && !is_array($optionValue))
                                    <input type="hidden" name="{{$pageName}}-fltr[{{$key}}][]"
