@extends('layouts.master', ['title'=>__('Logs')])
                        ...
                        @if($current_file)
                            <div class="float-right">
                                <a class="btn btn-sm btn-primary"
                                   href="?dl={{ \Illuminate\Support\Facades\Crypt::encrypt($current_file) }}"><span
                                        class="fa fa-download"></span>
                                    {{ __('Download') }}</a>
                                <a class="btn btn-sm btn-danger" id="delete-log" data-alert="{{ __('Are you sure?') }}"
                                   class="confirmation"
                                   href="?del={{ \Illuminate\Support\Facades\Crypt::encrypt($current_file) }}"><span
                                        class="fa fa-trash"></span>
                                    {{ __('Delete Current File') }}</a>
                                @if(count($files) > 1)
                                    <a class="btn btn-sm btn-danger" id="delete-all-log"
                                       data-alert="{{ __('Are you sure?') }}" class="confirmation"
            ...
            <div class="col-lg-12">
                <div class="box box-primary box-borderless">
                    <div class="box-body">
                        ...
                        @else
                            @component('components.table',['class' => 'lv-data-table data-table mb-4'])
                                @slot('thead')
                                ...
                                @foreach($logs as $key => $log)
                                    <tr data-display="stack{{{$key}}}">
                                        ...
                                        <td>
                                            @if ($log['stack'])
                                                <div class="stack d-block lf-pre-wrap"
                                                     id="stack{{{$key}}}">{{{ trim($log['stack']) }}}
                                                </div>
                                            @endif
                                        </td>
                                    </tr>
