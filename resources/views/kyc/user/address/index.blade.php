                    ...
                    <div class="nav-tabs-custom">
                        ...
                        <div class="p-4 bg-white border lf-toggle-border-color">
                                        ...
                                        <div class="address-dropdown-menu">
                                            <div class="dropdown-menu  drop-menu dropdown-menu-right">
                                                @if($address->is_verified == VERIFICATION_STATUS_UNVERIFIED )
                                                    @if(has_permission('kyc.addresses.edit'))
                                                        <a class="dropdown-item"
                                                           href="{{route('kyc.addresses.edit', $address->id)}}">
                                                            <i class="fa fa-edit mr-2"></i>
                                                            {{__('Edit')}}
                                                        </a>
                                                    @endif

                                                    @if(has_permission('kyc.addresses.destroy') && $address->is_default == INACTIVE)
                                                        <a class="dropdown-item confirmation"
                                                           ...
                                                           href="{{ route('kyc.addresses.destroy', $address->id) }}">
                                                            <i class="fa fa-trash-o mr-2"></i>
                                                            {{__('Delete')}}
                                                        </a>
                                                    @endif

                                                    @if(has_permission('kyc.addresses.toggle-default-status') && $address->is_default == INACTIVE)
                                                        <a class="dropdown-item confirmation"
                                                           data-alert="{{__('Are you sure to change the default status?')}}"
                                                           data-form-id="urm-{{$address->id}}" data-form-method='put'
                                                           href="{{ route('kyc.addresses.toggle-default-status', ['id' => $address->id]) }}"
                                                        >
                                                    ...
                                                    @if(has_permission('kyc.addresses.verification.create'))
                                                        <a class="dropdown-item"
                                                           href="{{route('kyc.addresses.verification.create', $address->id)}}">
                                                            <i class="fa fa-map-marker mr-2"></i>
                                                            {{__('Verify This Address')}}
                                                        </a>
                                                    @endif
                                                @elseif($address->is_verified == VERIFICATION_STATUS_PENDING)
                                                    @if(has_permission('kyc.addresses.verification.show'))
                                                        <a class="dropdown-item"
                                                           href="{{route('kyc.addresses.verification.show', [$address->id, $address->knowYourCustomer->id])}}">
                                                            <i class="fa fa-map-marker mr-2"></i>
                                                            {{__('View Document')}}
                                                        </a>
                                                    @endif
                            ...
                            <div class="text-center mt-4">
                                <a href="{{ route('kyc.addresses.create') }}" class="btn-sm btn-success">
                                    {{ __('Add New Address') }}
                                </a>
                            </div>
                        </div>
