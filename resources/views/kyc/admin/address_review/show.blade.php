            ...
            <div class="col-lg-12 mb-4">
                @component('components.card',['type' => 'info'])
                        ...
                        <div class="row justify-content-between">
                            ...
                            <div class="col">
                                <div class="card-tools">
                                    <a href="{{ route('kyc.admin.address-review.index') }}" class="btn btn-sm btn-info float-right back-button">
                    ...
                    <div class="row justify-content-center">
                        <div class="col-md-8">
                            @if(!is_null($verification->address))
                                @component('components.table', ['class' => 'table-borderless table-striped table-sm mt-3'])
                                    <tr>
                                        ...
                                        <td class="col-6">{{$verification->address->name}}</td>
                                    </tr>
                                    <tr>
                                        <td class="col-6"><i
                                                class="mr-1 fa fa-address-card-o"></i> {{__('Location')}}
                                        </td>
                                        <td class="col-6">{{$verification->address->city}} {{$verification->address->country->name}}</td>
                                    </tr>
                                    <tr>
                                        <td class="col-6"><i class="mr-1 fa fa-phone"></i> {{__('Phone')}}
                                        </td>
                                        <td class="col-6">{{$verification->address->phone_number}}</td>
                                    </tr>
                                    <tr>
                                        <td class="col-6"><i
                                                class="mr-1 fa fa-location-arrow"></i> {{__('Post Code')}}
                                        </td>
                                        <td class="col-6">{{$verification->address->post_code}}</td>
                                    </tr>
                                    <tr>
                                        <td class="col-6"><i class="mr-1 fa fa-check-circle"></i> {{__('Verification Status')}}</td>
                                        <td class="col-6">
                                            <span class="badge badge-{{config('commonconfig.verification_status.' . $verification->address->is_verified . '.color_class')}}">{{verification_status($verification->address->is_verified)}}</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="col-6"><i
                                                class="mr-1 fa fa-user"></i> {{__('Requested by')}}
                                        </td>
                                        <td class="col-6">{{$verification->address->ownerable_type == get_class(auth()->user()) ? 'Buyer' : 'Seller'}}</td>
                                    </tr>
                                @endcomponent
                            @endif
                        </div>
                        <div class="col-md-10">
                            ...
                            <div class=" py-4 text-center">
                                ...
                                @if(has_permission('kyc.admin.address-review.approve') && $verification->status !== VERIFICATION_STATUS_APPROVED)
                                    <a class="btn btn-info confirmation d-inline-block" data-alert="{{__('Are you sure?')}}"
                                       ...
                                       href="{{ route('kyc.admin.address-review.approve', ['id' => $verification->id]) }}">
                                ...
                                @if(has_permission('kyc.admin.address-review.destroy'))
                                    <a class="btn btn-danger confirmation d-inline-block ml-2" data-alert="{{__('Are you sure?')}}"
                                       ...
                                       href="{{ route('kyc.admin.address-review.destroy', $verification->id) }}">
