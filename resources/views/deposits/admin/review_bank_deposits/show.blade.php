...
@section('content')
                        ...
                        <div class="table-responsive">
                            <table class="table table-borderless">
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="badge badge-{{ get_color_class($deposit->status, 'payment_status') }}">
                                            {{ payment_status($deposit->status) }}</span>
                                    </td>
                                </tr>
                                <tr>
                                ...
                                <div class="card-footer lf-toggle-border-color text-right mt-3">
                                    @if(has_permission(replace_current_route_action('destroy')))
                                        <a class="btn btn-info"
                                           data-target="#deposit-amount-adjust-modal"
                                           data-toggle="modal"
    ...
    @if($deposit->status == PAYMENT_STATUS_REVIEWING)
        @include('deposits.admin.review_bank_deposits._amount_adjust_form')
    @endif
@endsection
        ...
        <script>
            (function ($) {
                ...
                modal.on("hidden.bs.modal", function () {
                    ...
                    if (formMessage.hasClass('alert')) {
                        formMessage.removeClass('alert');
                    }
                    if (formMessage.hasClass('alert-danger')) {
                        formMessage.removeClass('alert-danger');
                    }
                    if (formMessage.hasClass('alert-success')) {
                        formMessage.removeClass('alert-success');
                    }
                });
            }) (jQuery);
