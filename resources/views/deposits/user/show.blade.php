    ...
    <div class="container my-5">
        <div class="row">
            <div class="col-md-12">
                        ...
                        <div class="table-responsive">
                            <table class="table table-borderless">
                                <tr>
                                    ...
                                    <td>{{ auth()->user()->profile->full_name }}</td>
                                    <th class="pl-0">{{ __('Wallet') }}</th>
                                    <td>{{ $wallet->currency->name }} ({{ $wallet->currency_symbol }})</td>
                                    <th class="pl-0">{{__("Status")}}</th>
                                    <td>
                                        <span
                                            class="badge badge-{{ config('commonconfig.payment_status.'.$deposit->status.'.color_class') }}">{{ payment_status($deposit->status) }}</span>
                                    </td>
                                </tr>
                                <tr>
                ...
                @if($deposit->payment_method == PAYMENT_METHOD_BANK)
                    <!-- USER BANK DETAILS -->
                                ...
                                <table class="table table-borderless">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            {{ verification_status($deposit->bankAccount->is_verified) }}
                                            </span>
                                        </td>
                    ...
                    <!-- select system bank -->
                    @if($deposit->status == PAYMENT_STATUS_PENDING)
                        {{ Form::open(['url' => route('wallets.deposits.update', [$wallet->currency_symbol, $deposit->id]), 'method' => 'put', 'files' => true, 'id' => 'depositForm']) }}
                        <!-- USER BANK DETAILS -->
                            <div class="card p-4">
                                <div class="card-header px-0 pb-4 pt-0">
                                    <h3 class="w-card-title">
                                ...
                                <div class="card-body px-0 pt-4 pb-0">
                                    <!-- banks -->
                                    @if($systemBanks->count() > 0)
                                        <h5>{{ __('Select a bank to deposit') }}</h5>
                                        <div class="row">
                                            @foreach($systemBanks as $systemBank)
                                                <div class="{{ count($systemBanks) == 1? 'col-md-12': 'col-md-6' }}">
                                                    ...
                                                    ['id' => $systemBank->id,'class' => 'd-none bank-list']) }}
                                                        <div
                                                            class="card bank-card my-3 h-100">
                                                                ...
                                                                <div class="table-responsive">
                                                                    <table class="table table-borderless">
                                                                        <tr>
                                                                            ...
                                                                            <td>
                                                                                <span
                                                                                    class="mx-3">:</span> {{ $systemBank->bank_name }}
                                                                            </td>
                                                                        </tr>
                                                                        <tr>
                                                                            ...
                                                                            <td>
                                                                                <span
                                                                                    class="mx-3">:</span> {{ $systemBank->account_holder }}
                                                                            </td>
                                                                        </tr>
                                                                        <tr>
                                                                            ...
                                                                            <td>
                                                                                <span
                                                                                    class="mx-3">:</span> {{ $systemBank->reference_number }}
                                                                            </td>
                                                                        </tr>
                                                                        <tr>
                                                                            <th>{{  __("IBAN") }}</th>
                                                                            <td>
                                                                                <span
                                                                                    class="mx-3">:</span> {{ $systemBank->iban }}
                                                                            </td>
                                                                        </tr>
                                                                        <tr>
                                                                            <th>{{  __("SWIFT") }}</th>
                                                                            <td>
                                                                                <span
                                                                                    class="mx-3">:</span> {{ $systemBank->swift }}
                                                                            </td>
                                                                        </tr>
                                                                        <tr>
                                                                            <th>{{  __("Country") }}</th>
                                                                            <td>
                                                                                <span
                                                                                    class="mx-3">:</span> {{ $systemBank->country->name }}
                                                                            </td>
                                                                        </tr>
                                                                        <tr>
                                                                            <th>{{  __("Address") }}</th>
                                                                            <td>
                                                                                <span
                                                                                    class="mx-3">:</span> {{ $systemBank->bank_address }}
                                                                            </td>
                                                                        </tr>
                                                                    </table>
                                                                </div>
...
{{--                                        {{ Form::file('receipt', ['id' => 'receipt']) }}--}}
                            ...
                            <div class="form-group bank-deposit-btn">
                                {{ Form::submit(__("Submit"), ['class' => 'btn bg-info text-white btn-block btn-lg border-top-0 form-submission-button']) }}
                            </div>
                        {{ Form::close() }}
                    @endif
                    <!-- receipt -->
                        ...
                        <div class="card p-4">
                            ...
                            <div class="card-body px-0 pt-4 pb-0">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div class="d-flex">
                                            <div class="bg-light text-muted icon-box mr-3 align-self-start d-none d-sm-none d-md-flex">
                                                <i class="fa fa-bank m-auto"></i>
                                            </div>
                                            <figure>
                                                <img class="img-fluid"
                                                     src="{{ get_deposit_receipt($deposit->receipt) }}" alt="">
                                            </figure>
                                        </div>
                                    </div>
                                </div>
