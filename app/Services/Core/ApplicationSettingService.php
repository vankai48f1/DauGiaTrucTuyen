...
class ApplicationSettingService
{
    ...
    private function uploadImages($uploadAbleImages, &$updateAbleSettings)
    {
        ...
        foreach ($uploadAbleImages as $field => $file) {
            ...
            $width = isset($this->settingsConfigurations[$this->type]['sub-settings'][$this->subType][$field]['width']) ? $this->settingsConfigurations[$this->type]['sub-settings'][$this->subType][$field]['width'] : null;
            $height = isset($this->settingsConfigurations[$this->type]['sub-settings'][$this->subType][$field]['height']) ? $this->settingsConfigurations[$this->type]['sub-settings'][$this->subType][$field]['height'] : null;
    ...
    public function loadForm($settingGroup = null, $subSettingGroup = null, $viewOnly = false)
    {
        ...
        foreach ($settingGroup['sub-settings'][$subSettingGroup] as $key => $value) {
            ...
            $value_data = old($key, settings($key, isset($value['default']) ? $value['default'] : null));
            ...
            $startTag = isset($key['section_start_tag']) ? $key['section_start_tag'] : (isset($common_wrapper['section_start_tag']) ? $common_wrapper['section_start_tag'] : '');


            $startTag = str_replace('>', '', $startTag);
            $startTag .= ' data-name="' . $key . '"';

            if (isset($value['display_when'])) {
                $hidden = true;
                foreach ($value['display_when'] as $fieldName => $filedVal) {
                    $startTag .= ' data-' . $fieldName . '="|' . implode('|', $filedVal) . '|"';
                    if ($hidden) {
                        $relatedValue = old($fieldName, settings($fieldName, isset($settingGroup['sub-settings'][$subSettingGroup][$fieldName]['default']) ? $settingGroup['sub-settings'][$subSettingGroup][$fieldName]['default'] : null));
                        if (in_array($relatedValue, $filedVal)) {
                            $hidden = false;
                        }
                    }
                }
                ...
                if ($hidden) {
                    if (strpos($startTag, ' class="') > 0) {
                        $startTag = str_replace(' class="', ' class="d-none ', $startTag);
                    } else {
                        $startTag .= ' class="d-none"';
                    }
                }
            }
