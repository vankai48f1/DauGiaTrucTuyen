...
let cmbInputBuildingFuncs = {
    ...
    setStyle : function(targetDom, childItem, pseudo, childPseudo, styleName, value){
        let styleIndex = '.'+ targetDom.attr('data-cmb-wrapper') + '[data-cmb-id="'+targetDom.attr('data-cmb-id')+'"]' + (pseudo ? pseudo : '') + (childItem ? ' ' + childItem + (childPseudo ? childPseudo : '') : '');
    ...
    pseudoLoop : function(targetDom, options, pseudoValue, value, styleName){
        ...
        if (options.childItem){
            if (Array.isArray(options.childItem)){
                $.each(options.childItem, function (childItemKey, childItemValue) {
                    if (Array.isArray(options.childPseudo)){
                        $.each(options.childPseudo, function (childPseudoKey, childPseudoValue) {
                            $this.setStyle(targetDom, childItemValue, pseudoValue, childPseudoValue, styleName, value);
                        });
                    }
                    else {
                        $this.setStyle(targetDom, childItemValue, pseudoValue, options.childPseudo, styleName, value);
                    }
                })
            }
            else {
                if (Array.isArray(options.childPseudo)){
                    $.each(options.childPseudo, function (childPseudoKey, childPseudoValue) {
                        $this.setStyle(targetDom, options.childItem, pseudoValue, childPseudoValue, styleName, value);
                    });
                }
                else {
                    $this.setStyle(targetDom, options.childItem, pseudoValue, options.childPseudo, styleName, value);
                }
            }
        }
        else {
            $this.setStyle(targetDom, false, pseudoValue, false, styleName, value);
        }
    },
