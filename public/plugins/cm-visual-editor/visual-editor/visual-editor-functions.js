...
function cmbGetStyleValue(optionValue,targetDom,section,childItem, pseudo, childPseudo){
    ...
    if(!optionValue.param || Object.keys(optionValue.param).length === 1){
        let cssPropName = '.' + section + '[data-cmb-id="'+targetDom.attr('data-cmb-id')+'"]' + (pseudo ? pseudo : '') + (childItem ? ' '+ childItem + (childPseudo ? childPseudo : '') : '');
...
function cmbSummernoteEditFunc() {
    let el = $('#summernote');
    el.summernote({
        minHeight: 250,
        ...
        callbacks: {
            ...
            onCodeviewToggled: function () {
                if (el.summernote('codeview.isActivated')) {
                    el.summernote("code", cmbPrettierFormat(el.summernote("code")).replace(/<br \/>/g, ''));
                }
            }
        }
    });
